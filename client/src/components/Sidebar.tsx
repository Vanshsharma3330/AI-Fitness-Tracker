...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    {path: '/dashboard/food', label: 'Food', icon: UtensilsIcon},
    {path: '/dashboard/activity', label: 'Activity', icon: ActivityIcon},
  ...
  return (
    <nav className='hidden lg:flex flex-col w-64 bg-white dark:bg-slate-900 border-r-border-slate-100 dark:border-slate-800 p-6 transition-colors duration-200'>
      ...
      <div className='flex flex-col gap-2'>
        {navItems.map((item)=>(
          <NavLink key={item.path} to={item.path} end className={({isActive})=> `flex items-center gap-3 px-4 py-2.5 border-l-3 transition-all duration-200 ${isActive ? 'bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 dark:text-emerald-400 font-medium' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-700 dark:hover:text-slate-200 border-transparent'}`} >
